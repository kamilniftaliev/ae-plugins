...
let backupFile;
const icons = {
  addMarker: File("./icons/plus.png"),
  makeBackup: File("./icons/save.png"),
  skip: File("./icons/skip.png"),
  loadBackup: File("./icons/upload.png")
};

function buildUI() {
  const win = app.pluginsPanel;
  const panel = win.pluginsContainer.add("panel", undefined, "Cutter", {
    borderStyle: "black"
  });
  ...
  ui.recalculateFromBackup = panel.recalculate.add(
    "checkbox",
    undefined,
    "From Backup"
  );
  ...
  // Marker row //
  // ---------------------------------------- //
  panel.newMaker = panel.col1.add("panel", undefined, "Marker", {
    borderStyle: "black"
  });
  panel.newMaker.orientation = "row";

  ui.markers = panel.newMaker.add(
    "dropdownlist",
    undefined,
  ...
  // Add marker button
  ui.addMarkerBtn = panel.newMaker.add(
    "iconbutton",
    undefined,
    icons.addMarker,
    { style: "toolbutton" }
  ...
  // Skip button
  ui.skipBtn = panel.newMaker.add("iconbutton", undefined, icons.skip, {
    style: "toolbutton"
  });
  ...
  function addMarker(defaultProps = {}) {
    ...
    activeItemfootage
      .property("marker")
      .setValueAtTime(activeItem.time, newMarker);
  ...
  // Backup row //
  // ---------------------------------------- //
  panel.backup = panel.col1.add("panel", undefined, "Backup", {
    borderStyle: "black"
  });
  panel.backup.orientation = "row";

  // Make backup button
  ui.makeBackupBtn = panel.backup.add(
    "iconbutton",
    undefined,
    icons.makeBackup,
    {
  ...
  // Load backup button
  ui.loadBackupBtn = panel.backup.add(
    "iconbutton",
    undefined,
    icons.loadBackup,
    {
...
function getItemNames() {
  ...
  names.forEach(n => {
    const item = ui.footageName.add("item", n);
    if (n === selectedItemText) item.selected = true;
  });

  if (!selectedItemText) ui.footageName.items[0].selected = true;
...
function makeBackup(options = {}) {
  try {
    ...
    backup = {};
    backup.markers = getPropertyBackup(
      cutterComp.layer("footage").property("marker")
    );
    backup.updates =
      options.updates ||
      getPropertyBackup(cutterComp.layer("footage").effect("Updated").Checkbox);
...
function splitFootage() {
  ...
  videosName.map((videoName, index) => {
    ...
    if (videosName.length - nextIndex) {
      const nextPerc =
        perc + ((nextIndex / videosName.length) * 100 - perc) / 2;
...
function cut() {
  ...
  for (let i = 2; i < frames; i++) {
    ...
    if (updated) {
      ...
      Object.keys(videos)
        .filter(name => name !== videoName && name !== "all")
        .map(name => {
...
function createChangesComp() {
  ...
  changesComp = app.project.items.addComp(
    "Changes",
    footage.width,
    footage.height,
    footage.pixelAspect,
  ...
  changesLayer
    .effect("Difference Matte")
    .property("Difference Layer")
    .setValue(2);
}
...
function convertExpressionToKeyframes() {
  ...
  if (
    backup &&
    backup.updates &&
    backup.updates.times &&
    backup.updates.times.length
  ) {
  ...
  for (let i = start; i <= end; i += step) {
    ...
    win.progress.description(
      `Estimated time to analyze: ${minutes} ${seconds}`
    );
    ...
    updates.times.push.apply(updates.times, times);
    updates.values.push.apply(updates.values, values);
...
function createCutterComp() {
  ...
  if (cutterComp && cutterComp.layer("footage")) {
    ...
    cutterComp = app.project.items.addComp(
      "Cutter",
      footage.width,
      footage.height,
      footage.pixelAspect,
    ...
    footageLayer.applyPreset(
      File(`${path}/presets/cutter/updated-checkbox.ffx`)
    );
...
function clearEverything() {
  ...
  if (cutter && cutter.layer("footage")) {
    try {
      if (!backup) backup = {};
      backup.markers = getPropertyBackup(
        cutter.layer("footage").property("marker")
      );
...
function getFootageStartTime(name) {
  const markerProperty = getComp("Main")
    .layer("Camera")
    .property("marker");
  ...
  Object.keys(videos)
    ...
    .map((name, arrIndex) => {
      ...
      const newPosition = [
        ...
        layerPosition.value[1] + sizeToAdd
      ];
      ...
      footageInMainComp.applyPreset(
        File(`${path}/presets/cutter/footage-border.ffx`)
      );
      ...
      if (backup && backup.footages) {
        ...
        if (focusPoints) {
          focusTracker
            .property("position")
            .setValuesAtTimes(focusPoints.times, focusPoints.values);
        }
        ...
        if (timeRemaps) {
          compLayer
            .property("timeRemap")
            .setValuesAtTimes(timeRemaps.times, timeRemaps.values);
        }
      }
    });
